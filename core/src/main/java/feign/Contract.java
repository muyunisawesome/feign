    ...
    abstract class BaseContract implements Contract {
        ...
        public List<MethodMetadata> parseAndValidateMetadata(Class<?> targetType) {
            ...
            for (final Method method : targetType.getMethods()) {
                ...
                //这里可能变成SpringMVContract中->super.parseAndValidateMetadata回到下面84行
                //解析并验证元数据
        ...
        protected MethodMetadata parseAndValidateMetadata(Class<?> targetType, Method method) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (parameterTypes[i] == URI.class) { //这个基本不用了
                    data.urlIndex(i);
                    ...
                    if (data.isAlreadyProcessed(i)) {
                        ...
                        data.bodyIndex(i); //表示body是第几个位置
                        data.bodyType(Types.resolve(targetType, targetType, genericParameterTypes[i])); //boy类型
                    }
                }
            }
